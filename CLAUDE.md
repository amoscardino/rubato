...
- **Code-behind.** A component with any logic keeps it in a `X.razor.cs` partial class next to `X.razor`, so the `.razor` file is markup only. Injection moves with it: `@inject` becomes an `[Inject] private T Name { get; set; } = default!;` property. `@page` and `@inherits` stay in the `.razor` file — the razor-generated partial always emits a base class, so declaring one in the `.cs` partial too is a compile error, and the `.cs` partial is just `public partial class X`. Markup-only components (`Loader`, `PageTitleWithSuffix`) stay single-file.
...
- `Pages/` holds routable pages; `Components/` holds reusable components (`Components/Shared/` for cross-cutting ones). Root routes (`/`, `/day`, `/day/{date}`) all resolve to `Pages/Day.razor`. The day's table lives in `Day.razor` directly: as a separate component it was seven parameters, four of them relaying the page's own state and handlers straight back to it.
