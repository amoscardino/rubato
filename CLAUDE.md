...
- **`Error` is the one channel for "this didn't work."** Rows render it as a warning icon with the message as its tooltip; pages hand it to `PageShell` as an alert. A component that fails without throwing sets it directly, which is why the `"Not saved: "` prefix belongs to the save handlers and not to the markup — when the markup owned it, a clipboard failure read "Not saved: Could not copy to the clipboard." `Error` is per-component, so a self-contained child owns its own: `CopyButton` shows a failed clipboard write on itself rather than borrowing the row's slot.
...
- **The day table's column widths are stated twice** — the header row in `Day.razor` and the fields in `EntryRow.razor` — and nothing keeps them in sync, so change them together or the header stops lining up. Splitting the header into its own component would only move the two copies further apart.
